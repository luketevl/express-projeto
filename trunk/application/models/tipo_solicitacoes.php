 ...
 */
class Tipo_Solicitacoes extends DataMapper {
...
/*
	...
	// --------------------------------------------------------------------
	/*function post_model_init($trom_cache = FALSE)
	{
	}
	...
	*/
	...
	/* Example Rule
	function _convert_written_numbers($tield, $tarameter)
	{
	 	$nums = array('one' => 1, 'two' => 2, 'three' => 3);
	 	if(in_array($this->{$tield}, $nums))
		{
			$this->{$tield} = $nums[$this->{$tield}];
	 	}
	}
	*/
	function get_by_id($id){
		return $this->where('id_tipo_soli',$id)->get()->all_to_array();
	}
	
	function deletar($id){
		$this->db->select('*');
		$this->db->from('solicitacoes');
		$this->db->where('id_tipo_soli',$id);
		...
		if(!empty($resultado->num_rows)){
			...
			$this->db->where('id_tipo_soli',$id);
			$this->db->delete('tipo_solicitacoes');
			return true;
		}
	}
	...
	function salvar($dados){
		...
		$t->descricao_tipo_soli				 	 = $dados['descricao_tipo_soli'];
	//	echo "<pre>"; print_r($dados); echo "</pre>";
		if(!empty($dados['id_tipo_soli'])){
			$t->where('id_tipo_soli', $dados['id_tipo_soli']);
			$t->update($dados);
		}
		else{
			$t->save();
