 ...
 */
class Funcoes extends DataMapper {
...
/*
	...
	*/
	...
	function salvar($dados){
		$f = new Funcoes();
		$f->ativo				 	 = $dados['ativo'];
		$f->nome_func				 = $dados['nome_func'];
		$f->id_proj					 = $dados['id_proj'];
		$f->data_criacao_func		 = date('Y-m-d h:m:s');
		$f->id_proj					 = $dados['id_proj'];
		//echo "<pre>"; print_r($dados); echo "</pre>";
		if(!empty($dados['id_func'])){
			$f->where('id_func', $dados['id_func']);
			$f->update($dados);
		}
		else{
			$f->save();
			//echo $f->check_last_query();
		}
		return $f;
	}
}
