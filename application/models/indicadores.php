 ...
 */
class Indicadores extends DataMapper {
...
/*
	...
	*/
	function get_by_id($id){
		return $this->where('id_ind',$id)->get()->all_to_array();
	}
	
	function deletar($id_ind){
		$this->db->select('*');
	...
	function salvar($dados){
		$i = new Indicadores();
		$i->tipo_ind			 = $dados['tipo_ind'];
		$i->descricao_ind		 = $dados['descricao_ind'];
		$i->sigla_ind		 = $dados['sigla_ind'];
		$i->regra_ind		 = $dados['regra_ind'];
		$i->versao_ind		 = $dados['versao_ind'];
		$i->valor		 = $dados['valor'];
		$i->referencia_ind		 = $dados['referencia_ind'];
		$i->data_referencia_ind		 = $dados['data_referencia_ind'];
		//echo "<pre>"; print_r($dados); echo "</pre>";
		if(!empty($dados['id_ind'])){
			$i->where('id_ind', $dados['id_ind']);
			$i->update($dados);
		}
		else{
			$i->save();
			//echo $i->check_last_query();
		}
